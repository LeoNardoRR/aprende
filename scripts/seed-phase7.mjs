...
const journey = await one('learning_journeys', { network_id: networkId, program_id: program.id, curriculum_id: curriculum.id, skill_id: skills[0].id, title: 'Reconstruindo estratégias de cálculo - DEMO', description: 'Explicação, prática guiada e reavaliação em uma sequência validada.', difficulty: 'adaptive', estimated_minutes: 35, mastery_threshold: 70, status: 'published', current_version: 1, created_by: base.user_ids.teacher1, reviewed_by: base.user_ids.reviewer, approved_by: base.user_ids.approver, published_at: '2026-09-13T12:00:00Z' });
const steps = [];
...
await one('learning_journey_versions', { journey_id: journey.id, version_number: 1, snapshot: { journey, steps }, created_by: base.user_ids.approver });

const requestKey = '77000000-0000-4000-8000-000000000001';
const assignment = await one('learning_journey_assignments', { journey_id: journey.id, network_id: networkId, school_id: base.school_ids[0], classroom_id: base.classroom_ids[0], target_type: 'classroom', source_assessment_id: base.assessment_id, source_skill_id: skills[0].id, reason: 'Desempenho diagnóstico abaixo do limiar DEMO.', status: 'active', idempotency_key: requestKey, assigned_by: base.user_ids.teacher1 });
...
const manifest = { synthetic: true, network_id: networkId, resource_ids: [guide.id, reading.id], journey_id: journey.id, step_ids: steps.map((step) => step.id), assignment_id: assignment.id, recommendation_id: recommendation.id, ai_suggestion_id: aiSuggestion.id, fluency_activity_id: fluencyActivity.id, fluency_session_id: fluencySession.id, equity_group_id: equityGroup.id, expected_wcpm: 72 };
