...
export function AnalyticsDashboard({ mode, networks = [], schools = [], classrooms = [], fixedNetworkId, fixedSchoolId, fixedClassroomId, preview = false }: AnalyticsDashboardProps) {
  ...
  const load = useCallback(async () => {
    ...
    const result = await api.rpc<AnalyticsData>('get_analytics_dashboard', { filters });
    ...
    setData(result.data); setState(result.data.state as typeof state);
  }, [api, filters, preview]);
  ...
  return <section className={`analytics-dashboard ${mode === 'student' ? 'student-analytics' : ''}`} aria-labelledby={`analytics-title-${mode}`}>
    ...
    {mode === 'institutional' && <div className="analytics-filters" aria-label="Filtros dos indicadores">
      <label>Rede<select value={networkId} onChange={(event) => { setNetworkId(event.target.value); setSchoolId(''); setClassroomId(''); }}><option value="">Todas autorizadas</option>{networks.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
    ...
    {state !== 'loading' && state !== 'error' && state !== 'empty' && data && <>
      ...
      <div className="analytics-chart-grid">
        ...
        <figure><figcaption><strong>Evolução</strong><span>Médias comparáveis por avaliação e ordem temporal.</span></figcaption><div className="analytics-chart" role="img" aria-label="Gráfico de linha da evolução das avaliações"><ResponsiveContainer width="100%" height="100%"><LineChart data={data.evolution}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="assessment_title" /><YAxis domain={[0,100]} /><Tooltip /><Legend /><Line type="monotone" dataKey="percentage" name="Acerto (%)" stroke="#0d8a70" strokeWidth={3} /></LineChart></ResponsiveContainer></div><table><thead><tr><th>Avaliação</th><th>Resultado</th><th>Amostra</th></tr></thead><tbody>{data.evolution?.map((row) => <tr key={String(row.assessment_id)}><td>{row.assessment_title}</td><td>{number(row.percentage, '%')}</td><td>{number(row.observations)}</td></tr>)}</tbody></table></figure>
      </div>
