...
export function AccountSettings({
  ...
  async function updateDisplayName() {
    ...
    if (nextName.length < 2) {
      setNameNotice('Digite um nome com pelo menos 2 caracteres.');
      return;
    }
    setNameBusy(true);
    setNameNotice('Salvando nome...');
    if (preview) {
      onDisplayNameUpdated?.(nextName);
      setNameNotice('Nome atualizado na prévia.');
      setNameBusy(false);
      return;
    }
    const {
    ...
    if (!user) {
      setNameNotice('Sua sessão expirou. Entre novamente.');
      setNameBusy(false);
      return;
    }
    const { error: updateError } = await authClient
      ...
      .eq('id', user.id);
    if (updateError) setNameNotice(friendlySupabaseError(updateError.message));
    else {
      onDisplayNameUpdated?.(nextName);
      setNameNotice('Nome atualizado.');
    }
    setNameBusy(false);
  }
    ...
    try {
      ...
      setAvatarNotice(
        uploadError instanceof Error
          ? friendlySupabaseError(uploadError.message)
  ...
  async function removeAvatar() {
    ...
    const { error: profileError } = await authClient
      ...
      .eq('id', user.id);
    if (profileError)
      setAvatarNotice(friendlySupabaseError(profileError.message));
    else {
            ...
            >
              ...
              {role === 'teacher' && (
                <section className="account-name-card">
                  <div>
                    <h3>Nome exibido</h3>
                    <p>Este nome aparece no seu painel de professor.</p>
                  </div>
                  <div className="account-name-controls">
                    ...
                    <button
                      ...
                      disabled={nameBusy || name.trim() === displayName.trim()}
                  ...
                  {nameNotice && (
                    <output className="account-avatar-notice">
                      {nameNotice}
                    </output>
                  )}
                </section>
              ...
              {role === 'teacher' && (
                <section className="account-avatar-card">
                  ...
                  <div>
                    ...
                    <p>
                      A imagem será cortada ao centro e otimizada
                      automaticamente.
                    </p>
                    <div className="account-avatar-actions">
                      ...
                      {avatarUrl && (
                        <button
                          type="button"
                          className="teacher-secondary compact"
                          disabled={avatarBusy}
