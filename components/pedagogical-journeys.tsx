...
const demoStudentJourneys: StudentJourney[] = [{
  ...
  steps: [
    ...
    { id: 'demo-step-2', title: 'Praticar com apoio', instructions: 'Resolva os exercícios usando as pistas quando precisar.', step_type: 'exercise', position: 2, required: true, pedagogical_points: 10, gamification_points: 15, status: 'in_progress' },
    { id: 'demo-step-3', title: 'Verificar o que aprendeu', instructions: 'Faça a atividade final para registrar sua evolução.', step_type: 'reassessment', position: 3, required: true, pedagogical_points: 20, gamification_points: 20, status: 'not_started' },
  ],
}];
...
export function PedagogicalJourneys({ mode, networkId, schoolId, classroomId, skillId, preview = false }: { mode: Mode; networkId?: string | null; schoolId?: string | null; classroomId?: string | null; skillId?: string | null; preview?: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    if (mode === 'student') {
      const result = await api.rpc<StudentJourney[]>('get_my_learning_journeys');
      if (result.error) { setState('error'); setMessage(result.error.message); return; }
      setStudentJourneys(result.data ?? []); setState(result.data?.length ? 'success' : 'empty'); return;
    }
  ...
  async function completeStep(journey: StudentJourney, step: StudentStep) {
    ...
    const result = await api.rpc<{ progress_percentage: number }>('save_journey_step_progress', { target_assignment: journey.assignment_id, target_step: step.id, target_status: 'completed', response_payload: { completed_from: 'student_journey_ui' }, request_key: crypto.randomUUID() });
    if (result.error) { setMessage(result.error.message); return; }
  ...
  return <section id="remediation" className={`pedagogy-hub pedagogy-${mode}`} aria-labelledby={`pedagogy-title-${mode}`}>
    ...
    {state === 'success' && mode === 'student' && activeStudentJourney && <>
      ...
      <ol className="pedagogy-steps">{activeStudentJourney.steps.map((step) => <li key={step.id} className={step.status}><span>{step.status === 'completed' ? <CheckCircle2 /> : step.position}</span><div><small>{step.step_type}</small><strong>{step.title}</strong><p>{step.instructions}</p></div><button type="button" disabled={step.status === 'completed'} onClick={() => void completeStep(activeStudentJourney, step)}>{step.status === 'completed' ? 'Concluída' : step.id === nextStep?.id ? 'Continuar' : 'Iniciar'} <ArrowRight /></button></li>)}</ol>
