...
export function StudentConnect({ onClose }: { onClose: () => void }) {
    ...
    if (nextProfile?.role === 'student') {
      ...
      if (classIds.length) {
        ...
        if (nextAssignments.length) {
          ...
          setNotice(memberResult.error?.message ?? assignmentResult.error?.message ?? announcementResult.error?.message ?? submissionResult.error?.message ?? '');
        } else setSubmissions([]);
      } else { setAssignments([]); setSubmissions([]); setAnnouncements([]); }
  ...
  return <div className="student-connect-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}><section className="student-connect" role="dialog" aria-modal="true" aria-label="Área do aluno"><header><a className="teacher-logo" href="./"><span><GraduationCap size={24}/></span><strong>Aprendê</strong></a><button onClick={onClose} aria-label="Fechar"><X/></button></header>
    ...
    : !session ? <div className="student-connect-auth"><div><span className="teacher-kicker">SALA CONECTADA</span><h2>{creating ? 'Primeiro acesso' : 'Entre na sua conta'}</h2><p>{creating ? 'Crie seu acesso de aluno. Depois, use o código enviado pelo professor.' : 'Sua sessão fica salva neste navegador até você clicar em Sair.'}</p></div><section className="student-login-card"><div className="student-auth-tabs"><button type="button" className={!creating ? 'active' : ''} onClick={() => { setCreating(false); setNotice(''); }}>Entrar</button><button type="button" className={creating ? 'active' : ''} onClick={() => { setCreating(true); setNotice(''); }}>Primeiro acesso</button></div><form onSubmit={authenticate}>{creating && <label>Seu nome<input required minLength={2} maxLength={80} value={form.name} onChange={e => setForm({...form,name:e.target.value})} placeholder="Como o professor verá você"/></label>}<label>E-mail<input required type="email" autoComplete="email" value={form.email} onChange={e => setForm({...form,email:e.target.value})} placeholder="aluno@exemplo.com"/></label><label>Senha<input required minLength={8} type="password" autoComplete={creating ? 'new-password' : 'current-password'} value={form.password} onChange={e => setForm({...form,password:e.target.value})} placeholder="No mínimo 8 caracteres"/></label>{notice && <p className="teacher-notice" role="status">{notice}</p>}<button className="teacher-primary" disabled={busy}>{busy ? <LoaderCircle className="spin"/> : creating ? 'Criar minha conta' : 'Entrar'}</button></form></section></div>
