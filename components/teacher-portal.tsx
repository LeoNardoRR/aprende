...
function ClassForm({profile,onClose,onSaved}:{profile:Profile;onClose:()=>void;onSaved:()=>Promise<void>}) {
  ...
  async function submit(e:React.FormEvent<HTMLFormElement>){e.preventDefault();setBusy(true);const {error}=await supabase.from('classrooms').insert({owner_id:profile.id,name:form.name.trim(),subject:form.subject.trim()});setBusy(false);if(error)setNotice(friendlySupabaseError(error.message));else{await onSaved();onClose();}}
...
function AnnouncementForm({profile,classroom,onClose,onSaved}:{profile:Profile;classroom:Classroom;onClose:()=>void;onSaved:()=>Promise<void>}) {
  ...
  async function submit(e:React.FormEvent<HTMLFormElement>){e.preventDefault();setBusy(true);const {error}=await supabase.from('announcements').insert({classroom_id:classroom.id,author_id:profile.id,message:message.trim()});setBusy(false);if(error)setNotice(friendlySupabaseError(error.message));else{await onSaved();onClose();}}
