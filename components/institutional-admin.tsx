...
const previewInstitutionalClassrooms: ClassroomRow[] = [
  {
    ...
    network_id: 'network-preview',
    school_id: 'school-1',
    academic_year_id: 'year-preview',
    school_year_id: 'grade-1',
...
export function InstitutionalAdmin({
  ...
  const [networks, setNetworks] = useState<NetworkRow[]>(
    preview ? previewNetworks : [],
  );
  const [schools, setSchools] = useState<SchoolRow[]>(
    preview ? previewSchools : [],
  );
  const [academicYears, setAcademicYears] = useState<AcademicYearRow[]>(
    preview ? previewYears : [],
  );
  const [schoolYears, setSchoolYears] = useState<SchoolYearRow[]>(
    preview ? previewSchoolYears : [],
  );
  ...
  const loadData = useCallback(async () => {
    ...
    if (firstError)
      setNotice(
        `Não foi possível carregar toda a estrutura: ${firstError.message}`,
      );
  ...
  async function createRecord(event: FormEvent<HTMLFormElement>) {
    ...
    if (form === 'network') {
      const payload = {
        ...
        municipality: String(values.get('municipality') ?? '').trim() || null,
        state_code:
          String(values.get('state_code') ?? '')
            .trim()
            .toUpperCase() || null,
    ...
    if (form === 'grade') {
      const payload = {
        school_id: String(values.get('school_id')),
        name: String(values.get('name') ?? '').trim(),
        code: String(values.get('code') ?? '')
          .trim()
          .toUpperCase(),
  ...
  return (
    <div className="institutional-app">
      <a className="skip-link" href="#institutional-main-content">
      ...
      <aside className="institutional-sidebar">
        ...
        >
          <LogOut /> {signingOut ? 'Saindo…' : 'Sair'}
        </button>
      </aside>
      <main id="institutional-main-content" className="institutional-main">
        <header className="institutional-header">
          <div>
            <span>GESTÃO MUNICIPAL</span>
            <h1>Olá, {profile.display_name}</h1>
            <p>
              {roleLabels[profile.role]} · estrutura institucional da plataforma
            </p>
          </div>
          <div className="institutional-role">
            <ShieldCheck />
            <span>Escopo protegido por RLS</span>
          </div>
        </header>
          ...
          <>
            ...
            <section className="institutional-panel institutional-status">
              <div>
                ...
                <h2>{openYear?.label ?? 'Nenhum ano letivo aberto'}</h2>
                <p>
                  {openYear
                    ? `${formatDate(openYear.starts_on)} a ${formatDate(openYear.ends_on)}`
                    : 'Cadastre e abra um período para organizar turmas e matrículas.'}
                </p>
              </div>
              <CheckCircle2 />
            </section>
            ...
            <section id="schools" className="institutional-panel">
              <div className="institutional-panel-head">
                <div>
                  <span>ESTRUTURA</span>
                  <h2>Redes e escolas</h2>
                </div>
                {canConfigure && (
                  <div className="institutional-actions">
                    <button onClick={() => setForm('network')}>
                      <Plus /> Rede
                    </button>
                    <button
              ...
              <div className="institutional-school-grid">
                {networks.map((network) => (
                  <article key={network.id}>
                    <div className="institutional-network-title">
                      <Network />
                      <div>
                        <strong>{network.name}</strong>
                        <small>
                          {[network.municipality, network.state_code]
                            .filter(Boolean)
                            .join(' - ') || 'Município não informado'}
                        </small>
                      </div>
                    </div>
                    <div className="institutional-school-list">
                      {schools
                        .filter((school) => school.network_id === network.id)
                        .map((school) => (
                          <span key={school.id}>
                            <School />
                            <b>{school.name}</b>
                      ...
                      {!schools.some(
                        (school) => school.network_id === network.id,
                      ) && <em>Nenhuma escola cadastrada.</em>}
                    </div>
                  </article>
                ))}
                {!networks.length && (
                  <div className="institutional-empty">
                    <Network />
                    <h3>Comece pela rede de ensino</h3>
                    <p>
                      Crie a rede municipal para então vincular escolas, anos
                      letivos e usuários.
                    </p>
                  </div>
                )}
                ...
                {academicYears.map((year) => (
                  <article key={year.id}>
                    ...
                    <div>
                      ...
                      <span className={`institutional-badge ${year.status}`}>
                        {year.status === 'open'
                          ? 'Aberto'
                          : year.status === 'closed'
                            ? 'Encerrado'
                      ...
                      <small>
                        {formatDate(year.starts_on)} a{' '}
                        {formatDate(year.ends_on)}
                      </small>
                    </div>
                  </article>
                ))}
                {schoolYears.map((grade) => (
                  <article key={grade.id}>
                    <GraduationCap />
                    <div>
                      <strong>{grade.name}</strong>
                      <span className="institutional-badge">{grade.code}</span>
                      <small>
                        {schools.find((school) => school.id === grade.school_id)
                          ?.name ?? 'Escola'}
                      </small>
                    </div>
                  </article>
            ...
            <InstitutionalEnrollments
              ...
              schools={schools}
              academicYears={academicYears}
              classrooms={preview ? previewInstitutionalClassrooms : classrooms}
              preview={preview}
            />
            <InstitutionalBulk
              networks={networks}
            ...
            <div id="analytics">
              <AnalyticsDashboard
                ...
                schools={schools}
                classrooms={
                  preview ? previewInstitutionalClassrooms : classrooms
                }
                preview={preview}
              />
            </div>
            <PedagogicalJourneys
              ...
              classroomId={
                (preview ? previewInstitutionalClassrooms : classrooms)[0]?.id
              }
              preview={preview}
            />
      ...
      {form && (
        <div className="institutional-modal-backdrop" role="presentation">
          <section
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="institutional-form-title"
          >
            <div>
              <span>NOVO CADASTRO</span>
              <h2 id="institutional-form-title">{formTitle(form)}</h2>
            </div>
            <form onSubmit={createRecord}>
              {(form === 'school' || form === 'year') && (
                <SelectField
                  name="network_id"
                  label="Rede"
              ...
              {form === 'network' && (
                <>
                  <TextField name="name" label="Nome da rede" />
                  <TextField name="municipality" label="Município" />
                  <TextField name="state_code" label="UF" maxLength={2} />
                </>
              )}
              {form === 'school' && (
                <>
                  <TextField name="name" label="Nome da escola" />
                  <TextField name="code" label="Código da escola" />
                </>
              )}
              {form === 'year' && (
                <>
                  <TextField
                    name="label"
                    label="Identificação"
                    placeholder="Ano letivo 2027"
                  />
                  <TextField name="starts_on" label="Início" type="date" />
                  <TextField name="ends_on" label="Término" type="date" />
                  <SelectField
                    name="status"
                    label="Situação"
                    options={[
              ...
              {form === 'grade' && (
                <>
                  <TextField
                    ...
                    placeholder="6º ano"
                  />
                  <TextField name="code" label="Código" placeholder="6EF" />
                  <TextField name="sort_order" label="Ordem" type="number" />
                </>
              )}
  ...
  return (
    <label>
      ...
      <input
        name={name}
        type={type}
        required={name !== 'municipality' && name !== 'state_code'}
        {...props}
      />
    </label>
  );
